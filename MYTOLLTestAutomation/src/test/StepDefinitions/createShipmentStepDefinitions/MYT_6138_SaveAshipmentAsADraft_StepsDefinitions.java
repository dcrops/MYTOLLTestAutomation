...
public class MYT_6138_SaveAshipmentAsADraft_StepsDefinitions {
	...
	public void user_be_able_to_see_UI_prompt_displaying_the_Shipment_has_been_Saved_as_a_Draft(DataTable shipmentTestData) throws Throwable {
	...
	public void user_MUST_have_an_option_in_the_UI_prompt_to_Go_back_to_the_Create_Shipment_page(DataTable arg1) throws Throwable {
		  
		CreateShipmentActions.ClickGoBackBtn();
	}

	@Then("^User MUST have an option in the UI prompt to be re-directed to 'My Dashboard'$")
	public void user_MUST_have_an_option_in_the_UI_prompt_to_be_re_directed_to_My_Dashboard(DataTable arg1) throws Throwable {
		
		CreateShipmentActions.ClickGoToMyDashboardBtn();
	}

	@Then("^User can see pre-filled data available in 'Draft Shipments'$")
	public void user_can_see_pre_filled_data_available_in_Draft_Shipments(DataTable arg1) throws Throwable {
	
		System.out.println("Toll Carrier in Dashboard  "+CreateShipmentActions.GetDraftShipmentTollCarrierInMyDashboard());
	}

	@Then("^Draft Shipment MUST be saved under 'Draft Shipments' tab in 'My Dashboard'$")
	public void draft_Shipment_MUST_be_saved_under_Draft_Shipments_tab_in_My_Dashboard(DataTable arg1) throws Throwable {
	...
	public void user_can_see_all_data_in_the_draft_shipment_transferred_to_the_Create_Shipment_page(DataTable shipmentTestData) throws Throwable {

		for (Map<String, String> shipment : shipmentTestData.asMaps(String.class, String.class)) {
			  			
				assertEquals(shipment.get("TollCarrier").replaceAll("\\s", ""),BookAPickupActions.GetTollCarrier());
				assertEquals(shipment.get("AccountNumber").replaceAll("\\s", ""),CreateShipmentActions.GetService());
